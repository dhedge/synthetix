...
contract('StakingDualRewards', accounts => {
	...
	describe('Constructor & Settings', () => {
		it('should set rewards token on constructor', async () => {
			assert.equal(await stakingRewards.rewardsTokenA(), rewardsTokenA.address);
		});
	...
	describe('Function permissions', () => {
		...
		before(async () => {
			await rewardsTokenA.transfer(stakingRewards.address, rewardValue, { from: owner });
		});
	...
	describe('lastTimeRewardApplicable()', () => {
		...
		describe('when updated', () => {
			it('should equal current timestamp', async () => {
				await stakingRewards.notifyRewardAmount(toUnit(1.0), 0, {
	...
	describe('rewardPerToken()', () => {
		it('should return 0', async () => {
			assert.bnEqual(await stakingRewards.rewardPerTokenA(), ZERO_BN);
		});
		...
		it('should be > 0', async () => {
			...
			await rewardsTokenA.transfer(stakingRewards.address, rewardValue, { from: owner });
			await stakingRewards.notifyRewardAmount(rewardValue, 0, {
				from: mockRewardsDistributionAddress,
			});

			await fastForward(DAY);

			const rewardPerToken = await stakingRewards.rewardPerTokenA();
	...
	describe('earned()', () => {
		it('should be 0 when not staking', async () => {
			assert.bnEqual(await stakingRewards.earnedA(stakingAccount1), ZERO_BN);
		});
		...
		it('should be > 0 when staking', async () => {
			...
			const earned = await stakingRewards.earnedA(stakingAccount1);
		...
		it('rewardRate should increase if new rewards come before DURATION ends', async () => {
			...
			const rewardRateInitial = await stakingRewards.rewardRateA();
			...
			const rewardRateLater = await stakingRewards.rewardRateA();
	...
	describe('getReward()', () => {
		it('should increase rewards token balance', async () => {
			...
			const initialEarnedBal = await stakingRewards.earnedA(stakingAccount1);
			await stakingRewards.getReward({ from: stakingAccount1 });
			const postRewardBal = await rewardsTokenA.balanceOf(stakingAccount1);
			const postEarnedBal = await stakingRewards.earnedA(stakingAccount1);
	...
	describe('getRewardForDuration()', () => {
		it('should increase rewards token balance', async () => {
			...
			const rewardForDuration = await stakingRewards.getRewardAForDuration();

			const duration = await stakingRewards.rewardsDuration();
			const rewardRate = await stakingRewards.rewardRateA();
	...
	describe('exit()', () => {
		it('should retrieve all earned and increase rewards bal', async () => {
			...
			await rewardsTokenA.transfer(stakingRewards.address, totalToDistribute, { from: owner });
			await stakingRewards.notifyRewardAmount(toUnit(5000.0), 0, {
				from: mockRewardsDistributionAddress,
			});
	...
	describe('Integration Tests', () => {
		...
		it('stake and claim', async () => {
			...
			const rewardRewardsEarned = await stakingRewards.earnedA(stakingAccount1);
			...
			const rewardRewardsEarnedPostWithdraw = await stakingRewards.earnedA(stakingAccount1);
